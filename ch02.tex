...
To this end, fix an $n$, and fix an algorithm $A$ that finds a $3$-colouring in this setting. Let $t$ be the running time of the algorithm. In particular, the output of a node only depends on the information that is available within distance $t$ from it.

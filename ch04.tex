...
Recall the algorithm $\algo{P3C}$ for $3$-colouring paths from Section~\ref{sec:algo-p3c}. We will now present the algorithm in a formally precise manner as a state machine. Let us start with the problem definition:
\begin{itemize}[noitemsep]
    \item $\calF$ is the family of path graphs.
    \item $\Pi$ is the problem of colouring graphs with $3$ colours.
    \item $\Pi'$ is the problem of colouring graphs with any number of colours.
\end{itemize}
We will present algorithm $A$ that solves problem $\Pi$ on graph family $\calF$ given $\Pi'$. Note that in Section~\ref{sec:algo-p3c} we assumed that we have unique identifiers, but it is sufficient to assume that we have some graph colouring, i.e., a solution to problem $\Pi'$.
...
This algorithm does precisely the same thing as the algorithm that was described in pseudocode in Table~\ref{tab:algo-p3c}. It can be verified that this algorithm indeed solves problem $\Pi$ on graph family $\calF$ given $\Pi'$, in the sense that we defined in Section~\ref{ssec:def-solving-graph-problem}.

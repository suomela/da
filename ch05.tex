...
We already learned in Chapter~\ref{ch:intro-pos} that unique identifiers can be used to find a proper colouring of a path. In what follows, we will generalise this to arbitrary networks. We start with a generalisation of the simple strategy that we used in Section~\ref{sec:algo-p3c}. Let $x \in \NN$. We present an algorithm called ``$\algo{Greedy}$'' that reduces the number of colours from $x$ to
\[
    y = \max \{ x-1, \Delta+1 \},
\]
...
\begin{figure}
    ...
    \caption{Greedy colour reduction. The active nodes have been highlighted. Note that in the original colouring $f$, the largest colour was $99$, while in the new colouring, the largest colour is strictly smaller than $99$\mydash we successfully reduced the number of colours in the graph.}\label{fig:greedy}
\end{figure}
...
Algorithm $\algo{Greedy}$ does not need to know the number of colours $x$ or the maximum degree $\Delta$; we only used them in the analysis. We can take any graph, blindly apply algorithm $\algo{Greedy}$, and we are guaranteed to reduce the number of colours by one\mydash provided that the number of colours was larger than $\Delta + 1$.
...
In principle, we could use this strategy in the $\LOCAL$ model to find a \Dpocol{} of any graph. However, such an algorithm would be extremely slow. In the worst case, we may have a long path of nodes, with increasing identifiers (colours) along the path, and in such a graph the running time of the greedy strategy would be linear in $|V|$: in each iteration, only one of the nodes is a local maximum. In the next sections, we will develop an algorithm that is much faster.


\section{Directed Pseudoforests}

We will first study graph colouring in a seemingly simple special case: we are given a pseudoforest with a particular orientation. Once we have solved the special case, we turn our attention to the more general case of colouring bounded-degree graphs.
...
\section{Greedy Colouring in Directed Pseudoforests}\label{sec:dpgreedy}
...
Note that it was crucial that we applied $\algo{Greedy}$ to colouring $s$, instead of colouring $f$; see Figure~\ref{fig:dpcritical} for examples.
...
\section{Fast Colouring in Pseudoforests}\label{sec:dpbit}
...
\section{Fast Colouring in General Graphs}\label{sec:colour}
...
\begin{ex}[counting]
    ...
    \begin{subex}
        \item Prove that the counting problem cannot be solved on $\calF$ in the $\PN$ model.
        \item Design an algorithm that solves the counting problem on $\calF$ in the $\LOCAL$ model in time $O(|V|)$. Present the algorithm in a formally precise manner, using the definitions of Sections \ref{sec:pnn} and \ref{sec:distr-alg}.
        \item Prove that the counting problem cannot be solved in time $o(|V|)$ on $\calF$ in the $\LOCAL$ model.
        \item Prove that the counting problem cannot be solved on $\calF'$ in the $\LOCAL$ model.
    \end{subex}
\end{ex}
...
\begin{ex}[leader election]
    ...
    \begin{subex}
        \item Prove that the leader election problem cannot be solved on $\calF$ in the $\PN$ model.
        \item Design an algorithm that solves the leader election problem on $\calF$ in the $\LOCAL$ model.
    \end{subex}
\end{ex}

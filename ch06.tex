...
\begin{itemize}
    ...
    \item Each node $v$ with $a(v) = 0$ and $C(v) \ne \bot$ will set $a(v) \gets 1$ when it has received acknowledgements from each port of $C(v)$. In particular, if a node has $C(v) = \emptyset$, it can set $a(v) \gets 1$ without waiting for any acknowledgements.
\end{itemize}
It is straightforward to verify that the algorithm indeed works and it constructs a BFS tree in $O(\diam(G))$ rounds in the $\CONGEST$ model.
